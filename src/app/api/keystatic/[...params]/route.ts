...
// Force dynamic - skip static generation
export const dynamic = "force-dynamic";
export const runtime = "edge";

// Lazy load the handler to avoid build-time config validation
let handler: { GET: (req: NextRequest) => Promise<Response>; POST: (req: NextRequest) => Promise<Response> } | null = null;

async function getHandler() {
  if (!handler) {
    const { makeRouteHandler } = await import("@keystatic/next/route-handler");
    const { config, fields, collection, singleton } = await import("@keystatic/core");

    const keystatic = config({
      storage: {
        kind: "github",
